...
from routes.schemas import TaskCreate, TaskResponse 
...
async def create_task(new_task: TaskCreate, user_id: int, db: Annotated[Session, Depends(get_db)]):
...
async def update_task(task_id: int, updated_task: TaskResponse, db: Annotated[Session, Depends(get_db)]):
    task = db.execute(select(Task).where(Task.id == task_id)).scalar_one_or_none()
    if task is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task was not found")
